package surevil.lottery.bl.account;
...
import surevil.lottery.dao.account.UserInfoDao;
...
import surevil.lottery.security.jwt.JwtUser;
import surevil.lottery.security.jwt.JwtUserDetailsService;
import surevil.lottery.util.AESDecodeUtils;
...
public class UserBlServiceImpl implements UserBlService {
     ...
     */
    @Override
    public UserLoginResponse register(String username, String password, String role) {
        userDao.save(new User("", username, password, Role.getRole(role), "", new ArrayList<>()));
        JwtUser jwtUser = (JwtUser) jwtUserDetailsService.loadUserByUsername(username);
        String token = jwtService.generateToken(jwtUser, EXPIRATION);
        return new UserLoginResponse(token);
